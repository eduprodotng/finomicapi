...
async function waitForRetry(attempt) {
  const delay = Math.min(1000 * 2 ** attempt, 60000); // Exponential backoff up to 60s
  return new Promise((resolve) => setTimeout(resolve, delay));
}
  ...
  const messages = [
    {
      role: "system",
      content: `
You are Finomic AI’s Financial Inquiry Module. Your role is to assist users with general financial questions related to budgeting, spending, and money management.

Introduce yourself as Finomic AI and explain your purpose clearly. Always ask the user to provide relevant information such as recent bank statements, income details, or spending records before delivering any specific advice or insights.
