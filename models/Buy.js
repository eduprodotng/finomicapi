...
async function getBuyByUser(userId) {
  return Buy.find({ userId, isArchived: false }).sort({ createdAt: -1 }).exec();
}
...
async function archiveAllForUser(userId) {
  return Buy.updateMany({ userId }, { isArchived: true });
}

async function deleteAllInquiriesForUser(userId) {
  return Buy.deleteMany({ userId });
}
