...
async function getEdByUser(userId) {
  return Ed.find({ userId, isArchived: false }).sort({ createdAt: -1 }).exec();
}
...
async function archiveAllForUser(userId) {
  return Ed.updateMany({ userId }, { isArchived: true });
}

async function deleteAllInquiriesForUser(userId) {
  return Ed.deleteMany({ userId });
}
