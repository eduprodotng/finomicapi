...
const Credit = mongoose.model("Goal", CreditSchema);
...
async function getCreditByUser(userId) {
  return Credit.find({ userId, isArchived: false })
    .sort({ createdAt: -1 })
    .exec();
}
...
async function archiveAllForUser(userId) {
  return Credit.updateMany({ userId }, { isArchived: true });
}

async function deleteAllInquiriesForUser(userId) {
  return Credit.deleteMany({ userId });
}
