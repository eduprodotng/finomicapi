...
async function getInvestmentByUser(userId) {
  return Investment.find({ userId, isArchived: false })
    .sort({ createdAt: -1 })
    .exec();
}
...
async function archiveAllForUser(userId) {
  return Investment.updateMany({ userId }, { isArchived: true });
}

async function deleteAllInquiriesForUser(userId) {
  return Investment.deleteMany({ userId });
}
