...
async function getDefiByUser(userId) {
  return Defi.find({ userId, isArchived: false })
    .sort({ createdAt: -1 })
    .exec();
}
...
async function archiveAllForUser(userId) {
  return Defi.updateMany({ userId }, { isArchived: true });
}

async function deleteAllInquiriesForUser(userId) {
  return Defi.deleteMany({ userId });
}
