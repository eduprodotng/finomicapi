...
// const Inquiry = mongoose.model("FinInquiry", InquirySchema);
const Inquiry =
...
async function getInquiryByUser(userId) {
  return Inquiry.find({ userId, isArchived: false })
    .sort({ createdAt: -1 })
    .exec();
}
...
async function archiveAllForUser(userId) {
  return Inquiry.updateMany({ userId }, { isArchived: true });
}

async function deleteAllInquiriesForUser(userId) {
  return Inquiry.deleteMany({ userId });
}
