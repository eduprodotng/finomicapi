...
const Bnpl = mongoose.model("Bnpl", BnplSchema);

// Save new inquiry
async function saveBnpl({
  userId,
  chatTitle,
  chatId,
  userMessage,
...
async function getBnplByUser(userId) {
  return Bnpl.find({ userId, isArchived: false })
    .sort({ createdAt: -1 })
    .exec();
}
...
async function archiveAllForUser(userId) {
  return Bnpl.updateMany({ userId }, { isArchived: true });
}

async function deleteAllInquiriesForUser(userId) {
  return Bnpl.deleteMany({ userId });
}
